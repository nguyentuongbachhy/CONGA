...
std::vector<torch::Tensor> rope_fwd(
    ...
    if (sin.scalar_type() != q.scalar_type()) {
...
std::vector<torch::Tensor> rope_bwd(
    ...
    if (cos.scalar_type() != grad_q_out.scalar_type()) {
        cos = cos.to(grad_q_out.scalar_type());
    }
    if (sin.scalar_type() != grad_q_out.scalar_type()) {
        sin = sin.to(grad_q_out.scalar_type());
    }
