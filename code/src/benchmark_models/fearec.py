...
class FEARecModel(SequentialRecModel):
    ...
    def calculate_loss(self, input_ids, answers, neg_answers, same_target, user_ids):
        seq_output = self.forward(input_ids)  # (B, L, H)
        test_item_emb = self.item_embeddings.weight

        # Main CE loss. Seq mode supervises every non-padding position for
        # ~maxlen x speedup; prefix mode keeps last-position supervision.
        # info_nce / fredom branches below slice z[:, -1, :] themselves.
        if answers.dim() == 2:
            logits_full = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
            mask = (answers != 0)
            loss = nn.CrossEntropyLoss()(logits_full[mask], answers[mask].long())
        else:
...
class FEARecLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        # In the reference implementation q_index / k_index were populated via
        # `list(range(self.left, self.right))` and then broadcast into an empty
        # box one coefficient at a time in a Python loop, which dominates the
        # per-batch cost. Since the indices are a contiguous range we replace
        # the three O(len(q_index)) loops with a single slice + assignment.
        lo, hi = self.left, self.right
        q_fft_box = q_fft[..., lo:hi]
        k_fft_box = k_fft[..., lo:hi]
        ...
        if self.dual_domain:
            # time_q_index / time_k_index / time_v_index are also contiguous
            # ranges (either [lo, hi) when std=True or [0, L//2+1) otherwise),
            # so we can zero-fill the spatial tensors once and drop a slice in.
            if self.std:
                tq_lo, tq_hi = lo, hi
                tk_lo, tk_hi = lo, hi
                tv_lo, tv_hi = lo, hi
            ...
            spatial_q[..., tq_lo:tq_hi] = q_fft[..., tq_lo:tq_hi]

            spatial_k = torch.zeros(B, H, E, L // 2 + 1, device=k_fft.device, dtype=torch.cfloat)
            spatial_k[..., tk_lo:tk_hi] = k_fft[..., tk_lo:tk_hi]
            ...
            spatial_v[..., tv_lo:tv_hi] = v_fft[..., tv_lo:tv_hi]
