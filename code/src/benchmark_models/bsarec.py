...
from benchmark_models._modules import LayerNorm, FeedForward, MultiHeadAttention
...
class BSARecModel(SequentialRecModel):
    ...
    def calculate_loss(self, input_ids, answers, neg_answers, same_target, user_ids):
        ...
        if answers.dim() == 2:
            # Sequence-level: softmax over items at every non-padding
            # position. Equivalent to BSARec's last-position CE averaged
            # over L targets per user in a single forward pass.
            logits = torch.matmul(seq_output, item_emb.transpose(0, 1))  # (B,L,V)
            mask = (answers != 0)
            loss = nn.CrossEntropyLoss()(logits[mask], answers[mask].long())
            return loss

        # Legacy prefix path (last-position supervision).
        seq_last = seq_output[:, -1, :]
        logits = torch.matmul(seq_last, item_emb.transpose(0, 1))
